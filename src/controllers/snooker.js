...
export class Snooker {
		...
		if (this.whiteOnHand) {
			...
			if (!this.stopCueUpdates) {
				...
				if (this.game.input.activePointer.isDown) {
					let self = this;
					setTimeout(function () {
						self.audio.cueHitPlay();
						self.cue.kill();
						self.accelerateToObject(self.white.sprite, self.game.input.mousePointer, 80000);
					}, 100);
					setTimeout(function () { self.updateGame = true; }, 1000);
	...
	createGraphics() {
		...
		this.game.time.events.loop(Phaser.Timer.SECOND / 10000, this.updateBallsMovingState, this);
	}
